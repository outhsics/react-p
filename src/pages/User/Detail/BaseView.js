...
import { Form, Input, Upload, Select, Button, Progress, Row, Col, Table } from 'antd';
...
const columns = [
  ...
  },
  {
    ...
    render: pace => <span>{pace * 100}%</span>,
  },
  {
    title: '最新成绩',
    dataIndex: '最新成绩',
      ...
      <Fragment>
        ...
        <Row className={styles.progress}>
          ...
          <Col span={21}>
            {UserSpecial[0] && (
              <Progress
                className={styles.progress_r}
                percent={Number(pace * 100) || Number(UserSpecial[0].pace * 100)}
              />
            )}
          </Col>
        </Row>
            ...
            <Table
              ...
              footer={() => (
                <div className={styles.footer}>
                  <span className={styles.footerLeft}>试卷下共{userExam.length}份试卷</span>
                </div>
              )}
              rowKey={record => record.id}
              columns={columns}
              expandedRowRender={record => <p style={{ margin: 0 }}>缺少历史成绩</p>}
              dataSource={userExam}
            />
          )}
        </div>
